...
public class StackUtils {
  ...
  /**
   * Finds and removes the minimum element from the given stack based on the provided comparator.
   *
  ...
  public static <T> T min(Stack<T> stack, Comparator<? super T> comparator) {
    ...
    // find min value
    while (!stack.isEmpty()) {
      if (comparator.compare(stack.top(), min) < 0) min = stack.top();
      temp.push(stack.pop());
    }
  ...
  /**
   * Sorts the elements in the given stack in ascending order using the provided comparator.
   *
   * @param stack      the stack to be sorted
  ...
  public static <T> Stack<T> sort(Stack<T> stack, Comparator<? super T> comparator) {
    ...
    while (!temp.isEmpty()) sorted.push(min(temp, comparator));
    return sorted;
  }
  ...
  public static <T> boolean contains(Stack<T> stack, T item) {
    ...
    while (!temp.isEmpty()) {
      if (temp.pop().equals(item)) return true;
    }
    return false;
  }
  ...
  public static <T> void removeItems(Stack<T> stack, T item) {
    ...
    while (!stack.isEmpty()) {
      if (!stack.top().equals(item)) temp.push(stack.top());
      stack.pop();
    }
  ...
  public static <T extends Number> double sum(Stack<T> stack) {
    var temp = StackUtils.copy(stack);
    double sum = 0;
    while (!temp.isEmpty()) sum = sum + temp.pop().doubleValue();
    return sum;
  }
