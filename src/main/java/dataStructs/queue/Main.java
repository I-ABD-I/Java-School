...
public class Main {
    ...
    public static void splitUsingFirst(Queue<Integer> queue) {
        ...
        while (!queue.isEmpty()) {
            if (queue.head() > first)
                big.insert(queue.remove());
            else
                small.insert(queue.remove());
        }

        while (!small.isEmpty())
            queue.insert(small.remove());
        queue.insert(first);
        while (!big.isEmpty())
            queue.insert(big.remove());
    ...
    public static boolean isNoDiffBigger(Queue<Integer> queue, int diff) {
        ...
        while (!temp.isEmpty()) {
            ...
            while (!temp2.isEmpty()) {
                if (Math.abs(temp2.remove() - temp.head()) > diff)
                    return false;
            }
            temp.remove();
        }
    ...
    public static int countItemsBiggerThenOneAfter(Queue<Integer> queue) {
        ...
        var temp = QueueUtils.copy(queue);
        while (!temp.isEmpty()) {
            if (temp.remove() > (!temp.isEmpty() ? temp.head() : Integer.MAX_VALUE))
                count++;
        }
